...
# HTTP Request 헤더 설정
headers = {'Authorization': f'Bearer {sys.argv[1]}', 'X-GitHub-Api-Version': '2022-11-28', 'accept': 'application/vnd.github+json'}

# 백준허브 연동 알고리즘 Repository의 커밋 목록을 불러옴
api_response = json.loads(requests.get("https://api.github.com/repos/bmincof/Algorithms/commits", headers=headers).text)[0]
...
if difficulty in ['Silver', 'Gold']:
    ...
    requestBody = {'message': commit_message, 'committer': {'name': 'bmincof', 'email': 'bminlvcoffee@gmail.com'}, 'author': {'name': 'bmincof', 'email': 'bminlvcoffee@gmail.com'}, 'content': encoded_code }

    # 파일명 규칙 작성자/yyyy.mm/dd/
    response = requests.put(f'https://api.github.com/repos/bmincof/algostun/contents/bmincof/{filename}', headers=headers, json=requestBody )
    print(response.text)
